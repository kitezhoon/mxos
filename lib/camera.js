...
`;
    ...
    Camera.prototype.startRtsp = function () {
        ...
        if (this.config.MulticastEnabled) {
            ...
            if (this.config.RTSPServer == 2)
                this.rtspServer = utils.spawn("gst-variable-rtsp-server", ["-p", this.config.RTSPPort.toString(), "-m", this.config.RTSPName.toString(), "-u", "v4l2src device=/dev/video2 ! video/x-raw,width=640,height=480 ! v4l2h264enc ! rtph264pay name=pay0 pt=96"]);
        }
        if (this.rtspServer) {
